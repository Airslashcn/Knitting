...
scqO=(scqO-min(scqO))/(max(scqO)-min(scqO));
tO=[];
for i=1:pers
tO=[tO,tMean];
end
...
[~,pkScq]=findpeaks(PWP.FilterHigh(tMean,20,scqMean));
[~,pkSMG]=findpeaks(sMGMean);
...
scqO2=(scqO2-min(scqO2))/(max(scqO2)-min(scqO2));
figure(23)
subplot(2,2,1)
...
R2_1=PWP.ComputeR2(sMGMean,scqMean2);
for j=1:pers
    Rg1per=1+j*Nsamp-Nsamp:j*Nsamp;
    scq1per=scqDs2(Rg1per);
    
    R2_v(j)=PWP.ComputeR2(sMGMean,scq1per);
end
...
% figure(33)
% plot(ts,sMGDs,'linewidth',2);hold on
% plot(ts,scqDs,'linewidth',2);hold on
title([nameTitle,' R2:',num2str(R2_1),' R2bar:',num2str(var(R2_v)^0.5)],'fontsize',15)
fprintf('R2 %f,R2bar %f\n',R2_1,var(R2_v)^0.5)
hold off
% figure(33)
% plot(ts,sMGDs,'linewidth',2);hold on
% plot(ts,scqDs,'linewidth',2);